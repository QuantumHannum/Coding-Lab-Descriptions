...
1. Your scatter plot of $P(SAT)$ vs. $m$ should have a phase transition - i.e., it should illustrate a sudden drop in the probability of success over a small range of $m$.  What value of $m$ does this occur at according to your simulation?  Theoretically, this phase transition should occur at the \textit{Curie Point} as described here: \url{https://cse.hkust.edu.hk/tcsc/Seminar_Abstracts/Seminar_Slides/Diaz_SAT-sen_April_08.pdf}. There is evidence that for 3-SAT problems the transition should occur at $T=m/n=4.27$. Does your data match this prediction? Hypothetize about why or why not.
2. Building a class schedule for all TJ students is an annual process, and it never results in a schedule where everyone gets the classes they want.  Assuming 2200 students, each having 7 classes, and the information you gained in the previous question, comment on the existence of a ``perfect" schedule where everyone gets the classes they want.  In answering this question, you will need to estimate the number of logical conditions vs. the number of variables.
3. Your scatter plot time_plot.png is a window into the computational 'difficulty' in searching for a solution.  Looking at your scatter plot, what conclusions can you draw?  This paper \url{https://arxiv.org/pdf/cs/0005024} provides further detail on both the phase transition and computational 'difficulty' of a classical algorithm.  Does your data suggest a difference in the computational difficulty of the classical approach from the quantum approach?  Provide some substantive comments on the difference between classical SAT solvers and your implementation of Grover's Algorithm.  Use your plot depth_plot.png for some evidence.
4. This random 3-SAT problem on the surface seems disconnected from physics, but it has broad applications to thermodynamic problems.  Specifically, anything that can be modeled with an Ising Model \url{https://en.wikipedia.org/wiki/Ising_model}.  It even turns out that D-Wave (a quantum computing company) is making money by solving Ising-type problems.  Read this paper \url{https://www.dwavesys.com/media/vbklsvbh/weightedmaxsat_v2.pdf} to find out more about how 3-SAT problems are Ising Model problems.  See if you can find other examples online of interesting problems that can be solved with the Ising model.  Give a short description of why your chosen Ising-like problem is interesting to you. 
